...
def loaddata(fileNameTrain, fileNameTest):
    ...
    for line in file:
        ...
        try:
            ...
            '''
            # Use this if you want to train and test on the same scenario
            if flag == 0:
                # Train Dataset
                if label == 0 and count1 < 20001:
                    xdata.append([float(dur), protoDict[proto], int(Sport), int(
                        Dport), Sip, Dip, int(totP), int(totB), stateDict[state]])
                    ydata.append(label)
                    count1 += 1
                ...
                if label == 0 and count3 < 5001:
                    ...
                    xdataT.append([float(dur), protoDict[proto], int(Sport), int(
                        Dport), Sip, Dip, int(totP), int(totB), stateDict[state]])
                    ydataT.append(label)
                    count3 += 1
            ...
            '''
            # Train Dataset
            if label == 0 and count1 < 100001:
                xdata.append([float(dur), protoDict[proto], int(Sport), int(
                    Dport), Sip, Dip, int(totP), int(totB), stateDict[state]])
                ydata.append(label)
                count1 += 1

            elif label == 1 and count2 < 100001:
                xdata.append([float(dur), protoDict[proto], int(Sport), int(
                    Dport), Sip, Dip, int(totP), int(totB), stateDict[state]])
                ydata.append(label)
                count2 += 1

            elif count1 > 99999 and count2 > 99999:
                break
        except:
            continue
    ...
    for line in file:
        ...
        try:
            ...
            if "Background" in label:
             ...
             # Test Dataset
            if label == 0 and count3 < 10001:
                xdataT.append([float(dur), protoDict[proto], int(Sport), int(
                    Dport), Sip, Dip, int(totP), int(totB), stateDict[state]])
                ydataT.append(label)
                count3 += 1
            elif label == 1 and count4 < 10001:
                xdataT.append([float(dur), protoDict[proto], int(Sport), int(
                    Dport), Sip, Dip, int(totP), int(totB), stateDict[state]])
                ydataT.append(label)
                count4 += 1
            elif count3 > 9999 and count4 > 9999:
                break
        except:
            continue

    file = open('data/flowdata.pickle', 'wb')
    pickle.dump([np.array(xdata), np.array(ydata),
                np.array(xdataT), np.array(ydataT)], file)

    return np.array(xdata), np.array(ydata), np.array(xdataT), np.array(ydataT)


if __name__ == "__main__":
    loaddata('data/CTU-13-Dataset/1/capture20110810.binetflow',
